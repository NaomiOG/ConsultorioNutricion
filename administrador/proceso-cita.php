...
<head>
	...
	<script type="text/javascript" src="cita.js">
...
if(isset($_GET['E'])){
	...
	case 5:
		...
		$result="";
			...
			$result.= '<div class="alert alert-dismissible alert-danger" style="margin-left:24%; width:820px; font-family:Century Gothic; margin-top:10px;">
		  				<button type="button" class="close" data-dismiss="alert">&times;</button>
		  				<strong>Debes llenar todos los campos</strong> 
						</div>';
		...
		case 7:
			...
			case 9:
				...
				if(val()){
					...
					$cad="update medicamento set inventario=inventario-1 where id_medicamento=".$_POST['medicamento'];
					$objetoBD->consulta($cad);
			...
			$result="";
			function validar(){
				if(empty($_POST['desayuno'])||empty($_POST['almuerzo'])||empty($_POST['comida'])||empty($_POST['colacion'])||empty($_POST['cena'])||empty($_POST['alimentos'])||empty($_POST['indicaciones'])){
					return false;
				}
				else{
					return true;
			...
			if(validar()){
				...
				$cad="insert into dieta set id_paciente=".$_POST['id_paciente'].", id_cita=".$_POST['id_cita'].", desayuno='".$_POST['desayuno']."', almuerzo='".$_POST['almuerzo']."', comida='".$_POST['comida']."', colacion='".$_POST['colacion']."', cena='".$_POST['cena']."', alimentos='".$_POST['alimentos']."', indicaciones='".$_POST['indicaciones']."';";
					$objetoBD->consulta($cad);
					$result.="<script type='text/javascript'>";
	    			$result.='window.location="proceso-cita.php?E=2&id='.$_POST['id_cita'].'&pid='.$_POST['id_paciente'].'";';
	    			$result.='$.alert("Se han almacenado correctamente los cambios");';
			...
			if(validar()){
				...
				$cad="update dieta set desayuno='".$_POST['desayuno']."', almuerzo='".$_POST['almuerzo']."', comida='".$_POST['comida']."', colacion='".$_POST['colacion']."', cena='".$_POST['cena']."', alimentos='".$_POST['alimentos']."', indicaciones='".$_POST['indicaciones']."' where id_paciente=".$_POST['id_paciente']." and id_cita=".$_POST['id_cita'].";";
					$result.=$cad;
					$objetoBD->consulta($cad);
					$result.="<script type='text/javascript'>";
